...
const MUTATIONS = [
  ...
  [
    "src/core/blinkDrift.ts",
    "MIN_BLINKS_PER_HALF = 15",
    "MIN_BLINKS_PER_HALF = 14",
    "drift floor lowered: a half decided by fourteen blinks would be published beside one decided by a session",
    "test/core/blinkDrift.test.ts",
  ],
  [
    "src/core/blinkDrift.ts",
    "MIN_BLINKS_PER_HALF = 15",
    "MIN_BLINKS_PER_HALF = 16",
    "drift floor raised: a sitting with exactly fifteen blinks a half would be refused a number it can support",
    "test/core/blinkDrift.test.ts",
  ],
  [
    "src/core/blinkDrift.ts",
    "changeFraction: lateMean / earlyMean - 1",
    "changeFraction: lateMean - earlyMean",
    "the change becomes an absolute difference, which smuggles blink length back into a measure of change",
    "test/core/blinkDrift.test.ts",
  ],
  [
    "src/core/blinkDrift.ts",
    "const midpointMs = last / 2",
    "const midpointMs = last / 3",
    "the halves stop being halves: the boundary moves and each side stops describing the stretch it names",
    "test/core/blinkDrift.test.ts",
  ],
  [
  ...
  [
    "src/core/closureTaxonomy.ts",
    "MICROSLEEP_RANGE_MIN_MS = 2000",
    "MICROSLEEP_RANGE_MIN_MS = 1960",
    "microsleep-range edge lowered one frame period: a two second closure would be named for the band above it",
    "test/core/closureTaxonomy.test.ts",
  ],
  [
    "src/core/closureTaxonomy.ts",
    "MICROSLEEP_RANGE_MIN_MS = 2000",
    "MICROSLEEP_RANGE_MIN_MS = 2040",
    "microsleep-range edge raised one frame period: a closure past two seconds would still read prolonged",
    "test/core/closureTaxonomy.test.ts",
  ],
  [
    "src/core/closureTaxonomy.ts",
    "SUSTAINED_CLOSURE_MIN_MS = 15000",
    "SUSTAINED_CLOSURE_MIN_MS = 14960",
    "sustained edge lowered one frame period: a fifteen second closure would leave the microsleep range early",
    "test/core/closureTaxonomy.test.ts",
  ],
  [
    "src/core/closureTaxonomy.ts",
    "SUSTAINED_CLOSURE_MIN_MS = 15000",
    "SUSTAINED_CLOSURE_MIN_MS = 15040",
    "sustained edge raised one frame period: a closure past fifteen seconds would still be named for the microsleep range",
    "test/core/closureTaxonomy.test.ts",
  ],
  [
    "src/core/closureTaxonomy.ts",
    ...
    "if (durationMs < MICROSLEEP_RANGE_MIN_MS)",
    "band edge flipped to the upper side: exactly two seconds would change bands, and the project's one edge convention would hold in three places out of four",
    "test/core/closureTaxonomy.test.ts",
  ],
  [
    "src/core/closureTaxonomy.ts",
    ...
    "if (durationMs < LONG_CLOSURE_THRESHOLD_MS)",
    "blink edge flipped: a closure of exactly the blink maximum would be counted by blink.ts AND named here, breaking the one-closure-one-category partition",
    "test/core/closureTaxonomy.test.ts",
  ],
  [
    "src/core/constants.ts",
