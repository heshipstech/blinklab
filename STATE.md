**THE CHANGELOG WAS THE ONE DOCUMENT NOBODY THOUGHT TO GUARD, 7 September 2026 (row 10.0b4, ladder B17) — it had been publishing a twice-superseded Eyeblink8 headline for three weeks, in the present tense, in the section that says what is about to ship.** `CHANGELOG.md`'s Unreleased section said "87.7% recall, 83.3% precision, 85.4% F1" from 15 August. Those are the THIRD of the five figures this project has published for that benchmark; the current run is 83.6, 84.0 and 83.8. README keeps all five in a table and marks which is current, the result file keeps superseded runs below an explicit boundary line, and every other place a number appears is read back from the record by a test. The changelog was joined to none of it. The reason it was missed is worth naming and is not carelessness: a changelog reads like history, and history is exactly what this project deliberately does not rewrite, so a habit that is right everywhere else was applied to the one section of the one file where it is wrong. Unreleased is not history. It is a claim about the present living inside a historical document. `tools/changelogGuard.mjs` reads the three numbers back through `parseResultFile`, which 10.0b2 had already anchored to the block above `--- END OF THE CURRENT RUN ---`, so the pin follows the headline rather than the first match in a file of history. `statedEyeblink8` takes all three numbers or refuses by name, because a headline missing its F1 is a sentence somebody edited and reporting two thirds of one leaves the third drifting unwatched. 7 tests watched failing first. Three mutations, and the third is the one that matters: restoring the old headline only shows the pin exists, but moving the RESULT FILE's current run and watching the changelog test go red shows the direction of the dependency, that the document now has to follow the record rather than agree with a copy of it that was true once. guardsArmed goes 16 to 17. The suite is 1336 unit tests, 35 end to end tests and 498 Python tests of which 2 skip, all green. Next: 10.0b's remainder, the read-in-full stamp, ADR-0005 with PROJECT.md's four contradicting sentences, and the generated STATUS block.

**THE EXPLANATION SATISFIED THE GUARD THE EXPLANATION WAS ABOUT, 7 September 2026 (row 10.0b3, ladder B14 and D13) — two summary sentences had drifted from the files that decide them, in opposite directions, and the pin written to catch that could be talked past by its own paragraph.** CONTRIBUTING hands a contributor one command line and calls it the gates; `.github/workflows/ci.yml` decides what the gates are. `npm run counts:check` went into continuous integration in 10.0b1 and never followed into the document, so the documented line passed locally and a first pull request would fail on a step nobody had been told to run. README says which phases are complete; ROADMAP decides that, and README was a phase BEHIND, because Phase 9 finished and the sentence stayed put — a summary can be wrong in the modest direction too, and nothing notices, because nobody double-checks a claim that undersells. `tools/gateGuard.mjs` holds both, and counting complete phases meant encoding what this file means by complete: three settled markers, `[x]` done, `[~]` declined, and a `[ ]` row whose text opens `(moved to`, matched narrowly at the head of the row so ordinary prose about moving something cannot settle a row nobody moved. A guard that knew ticks alone would have called a finished phase unfinished forever. The find worth keeping is the hole dug and filled inside this row. The first reader took commands from fenced blocks AND inline code spans, because the install command was written inline; the paragraph added beside the fix names `npm run counts:check` in backticks while explaining the drift, so the pin was satisfied by its own explanation and deleting the gate from the command line left the suite green. Only the mutation caught it. The rule that came out: a gate counts as documented where it is RUN, not where it is discussed, and the install command moved into a block of its own rather than the reader being taught to read prose. That is the fourth guard this project has found that could not fail, and the first it wrote and broke in one afternoon. 13 tests watched failing first; three mutations — a gate dropped from the command line, a README claiming one phase too many, and a new gate added to continuous integration alone — each redden their own test. The parent row's file-count pin is dropped and said to be dropped: no document here states a file count, so there was nothing to hold. The suite is 1329 unit tests, 35 end to end tests and 498 Python tests of which 2 skip, all green. Next: 10.0b's remaining half, the stamp, the CHANGELOG guard, ADR-0005 and the STATUS block.

**THE CARD DESCRIBED EVERYTHING ABOUT THE INSTRUMENT EXCEPT WHAT RAN IT, 7 September 2026 (row 10.1g, ladder D4's provenance half) — every published Eyeblink8 number came out of a real browser, and MODEL_CARD did not say which one.** The card pinned the model file by hash, the vision runtime by the lockfile's version and integrity, and the five landmarker options parsed out of the source, each recomputed by a test from the committed artifacts. It said nothing about the browser, though `tools/measure_corpus.mjs` steps real clip files in one that Playwright launches, and a Playwright bump replaces the driver and the browser binary underneath it without one line of this repository changing. The gap was visible only from the other side: 10.1g4 had already added `package-lock.json` to the detector ratchet on exactly that reasoning, and once the lockfile is part of the instrument, what the card says about it answers itself. A new instrument section states the engine the runner actually launches, the `@playwright/test` version the lockfile pins, and the WebKit and Chromium versions the pinned `playwright-core` bundles, all read back by tests from the lockfile and from that package's own manifest rather than from a comment. Writing it turned up the part that cannot be fixed: the browser that produced the 25 and 26 August runs was never recorded, and the reproduction is written up as spanning "a different processor generation, operating system and WebKit binary", which is the sentence of somebody who knew the binary differed and did not write down either version. The section says so plainly and puts the next run's browser on the record before the measurement instead of reconstructing it after. `runnerEngine` refuses rather than guesses when a source launches nothing, two engines, or one it never imported, because a card naming a browser no measurement ran in is worse than one naming none; the card's patterns tolerate a line break between a name and its value, because the card is hard-wrapped prose and a guard that depends on where a paragraph wrapped fails for the wrong reason. Dependabot now excludes Playwright from its grouped minor-and-patch updates, so a bump that can move a published measurement arrives as its own pull request. The Check is met twice over: a bump that leaves the card behind reddens the build, and the ratchet separately demands it be declared or re-measured. 11 tests watched failing first; three mutations, a bumped lockfile version, a runner switched to Chromium and Playwright put back inside the group, each redden their own test. Row 10.1g is now complete across all three of its halves. The suite is 1313 unit tests, 35 end to end tests and 498 Python tests of which 2 skip, all green. Next: 10.0b (the stamp pins, the CHANGELOG guard, ADR-0005 and the STATUS block).

**THE CHEAPER FIX WAS THE ONE THAT SWITCHED OFF A REFUSAL, 7 September 2026 (row 10.14d) — the probe budget was thinnest at exactly the rate the protection it guards was written for, and the obvious way to fix that made the stepper blind to a bad clip.** Calibration may spend a fixed number of seeks to gather twelve frames, and row 10.14c left the cost measured at one rate. It is now reported: `summary.calibrationAttempts` travels with the summary beside `inexactLandings`, because a cost nobody can see is a cost nobody notices rising. The swept arithmetic floor is committed as a table in `docs/stepper-probe-budget.txt`. P1, P2 and P3 failed together: the cost does not rise with rate and is not flat, it FALLS, 51 attempts of a 60 budget at 24 frames per second against 20 at 300, because the quarter rule takes its step from a raw gap that at slow rates already is one period while at fast rates the 2 ms floor binds instead. The finding none of the predictions anticipated sits at the bottom of the table: sixty attempts could not gather twelve frames at 20 frames per second or below, gathering eleven at 20 and eight at 15 and calibrating on those, so the twelve-frame rule that makes one short gap a minority — raised from six by the September audit's critical finding, a 20 frames per second clip reported as 40 — was thinning out at exactly the rates it was written for. Three options were measured before one was chosen. The rule adapts: once the landings reduce under the whole-multiple rule the step is half of THAT period, with the quarter of the smallest raw gap kept for landings that do not reduce, where inventing a period would be the mistake the module refuses; that takes 30 frames per second from 48 attempts to 41 and 120 from 42 to 23. The budget rises to 120, justified by the measured floor of 81 at 15 frames per second rather than rounded up. Lifting the probe cap was measured and REJECTED, and that is the row's real find: it brings 24 frames per second from 51 attempts to 30 and solves the slow end outright, and it switches off a refusal, because the larger probe strides over an anomaly, so a 201 frame clip carrying one frame 25 ms out of place reports 200 frames at a confident 25 frames per second with zero inexact landings where it used to refuse by name as variable rate. The cap's stated reason was that a slow clip must not make the probe leap; its real job is keeping the probe small enough to land on anomalies, and that reason is now written where the cap is. Three mutations, each reddening its own tests: restoring the quarter rule, restoring the sixty-attempt budget, and lifting the cap. The neutrality argument for Eyeblink8 is a measurement rather than a reach argument, which is available here because the thing that could move is a number a test can read: all thirteen swept rates from 15 to 300 calibrate to an identical interval before and after, to the last digit a double carries, and the schedule a run walks comes from that interval and the origin alone. The suite is 1301 unit tests, 35 end to end tests and 498 Python tests of which 2 skip, all green. Next: 10.1g's provenance half, or 10.0b's stamp pins.

**THE PREDICTION WAS WRONG ABOUT WHERE, WHICH IS WHY IT WAS WRITTEN DOWN, 7 September 2026 (row 10.14c) — the stepper could not measure a clip above 200 frames per second at all, and the row that found it had named the wrong mechanism.** The probe step was a constant 10 ms. The row, written in amendment 17, said that is one whole frame period at 100 frames per second, so the probe lands on every other frame and calibration returns half the rate; its prediction, committed before any code, said a 120 fps clip would be measured at half. It is measured exactly right, and was before the change: a 10 ms step on a 120 fps clip skips frames unevenly, and ladder A1's whole-multiple rule, which landed after the row was written, recovers the true period from a mixture of one and two period gaps exactly. P1 failed and is recorded as failed. Sweeping the rate with clip length held at one second and again at three found the real boundary: 100, 120, 150 and 200 all correct before and after; 240 and 300 measured ZERO frames and returned no interval, because by then the constant probe is more than two periods and calibration cannot gather enough distinct landings. So the instrument was not publishing a confident wrong rate for a fast clip, it was visibly failing to measure one, which is a better failure than the row assumed and still a failure. `probeStep` in core now derives the step from the landings, a quarter of the smallest gap seen, floored at 2 ms and never above the bootstrap, and the stale comment claiming "4 ms is under half a frame even at 120" is gone. A half was measured too, fixes every rate, and costs half the attempts; it was not taken, because half of a gap that is itself two periods is exactly one period, which is the pathology being escaped. The price is attempts at ordinary rates, 38 to 45 of a 60 budget at 30 fps, and it has its own row 10.14d rather than a note. Mutation: restoring the constant reddens the 240 and 300 cases. 30 and 60 unchanged, which is why Eyeblink8 at 30 is expected unmoved, scoreable only by the owner's regression run at 10.8a, which now scores two stepper changes. The suite is 1290 unit tests, 35 end to end tests and 498 Python tests of which 2 skip, all green. Next: the owner's rows, or 10.14d.

**FIRST MATCH IS A GUESS WHEN THE FILE KEEPS ITS HISTORY, 7 September 2026 (row 10.0b2, ladder B14's parsing half) — the published precision was one regeneration away from coming out of a superseded run, and nothing downstream would have noticed.** `docs/eyeblink8-result.txt` holds four blocks with a headline in the same shape, because this project does not delete numbers it has published. `parseResultFile` took the first match anywhere in it, which is right only while the current block keeps matching. Row 10.10c1, already merged, changed the scorer to print a confidence interval inside the parentheses of the recall and precision lines; the expressions ended at `found)` and `invented)`, so a regenerated block would no longer have matched and the search would have run on to the 20 August run, reading precision 81.4% and 78 invented in place of 84.0% and 65. Recall would have stayed correct, because that superseded block carries an identical recall line, so a current recall would have been published beside a superseded precision with every byte comparison green, both sides reading the same wrong line. Reproduced as a test before the fix. Two changes: the expressions now tolerate extra text inside the parentheses, so this particular break does not happen, and the file carries an explicit `--- END OF THE CURRENT RUN ---` line with the generated figures read only above it and the parser refusing a file that lacks it rather than reading on. The second matters more, because it removes the assumption about document order that nothing was holding. Mutations: restoring the unbounded search and restoring the intolerant pattern each redden their own test. Nothing was live: it was armed, and it would have fired when the regenerated block from 10.8a's regression run was pasted in, which is when this file is least likely to be read closely. It was found by widening the detector ratchet in 10.1g4, by a reader who chose to verify a scorer change by running both versions and diffing their output rather than by reading the diff. This entry also repairs two things the two-session collision left in this file: the row 11.0a entry had a fragment of a deleted sentence spliced into its tail, and the row 10.1g3 entry appeared twice, once with a count that was never true of it. There is one of it now, below row 10.1g4, carrying the 1240 that was the suite when it was written. The suite is 1279 unit tests, 35 end to end tests and 498 Python tests of which 2 skip, all green. Next: 10.14c (the stepper's probe below half a period at high rates), whose prediction is committed before its code.

**THE PARTICIPANT WAS PART OF THE INSTRUMENT, 7 September 2026 (row 11.0a, ladder C4) — every live claim about catching blinks rests on a person pressing a button correctly while doing the thing being measured, and the ground truth moves into the app.** The count-ten protocol asks somebody to press Mark, blink ten times counting out loud, and press Mark again. What that cost is already recorded in `docs/validation-round.txt`: markers stamped up to about a second early, two of the first three people pressing Mark three or four times instead of twice, and one verdict the table had to refuse to call because a detection sat 0.46 seconds inside the marker slack and a shift could have changed the count. None of it is a fault in the people; it is a protocol that made them the timing reference for their own eyes. `src/core/cueSchedule.ts` fixes the schedule before any camera runs, on the `lightSchedule.ts` pattern: ten blink cues, a 3 second closure, a 20 second closure, a look-away, a rest after each, and a settle set to `BASELINE_LEARN_MS` rather than a tidy round number, because a cue delivered before the baseline is ready would be scored against a detector holding no line at all. `scoreCues` turns a session into caught, missed and latency, and refuses one that ended before the schedule did, ran far past it, or carried an event from before the first cue. Writing the tests first caught two design mistakes before anything ran, both the same shape: a scorer producing a confident number about the wrong thing. The first was conflation — an untyped event stream let a 120 ms blink count as answering "close your eyes for twenty seconds", and the tally came out three too high. Events carry their detector now and a cue is only answered by the detector that could have seen it. The second was scoring something invisible: nothing in the blink or closure path sees a person turning away from the screen, gaze does and this scorer does not read gaze, so a look-away that produced no blink would have read as a detector miss and quietly lowered every catch rate. It is excluded from the tally with the reason written where the exclusion happens. A third decision was about what not to reduce: a closure cued at twenty seconds and measured at eighteen is a two second error rather than a miss, so the score reports the signed error and no pass mark throws that number away. The response window is two seconds, pre-registered and stated as a CHOICE rather than dressed up as a derivation, bounded below by `MAX_BLINK_DURATION_MS` and above by the rest after each cue, with every latency reported so it can be re-chosen from real sessions by dated amendment rather than fitted to the answer. The limit is stated rather than hidden: this scorer cannot tell somebody following the cues from somebody blinking every 200 ms, because the second person does respond to every cue and no window arrangement separates them, so `eventsSeen` travels beside the tally. The two-marks protocol is retired at the top of `docs/participant-instructions.md`, with its reason and with the note that sessions already recorded stay valid for what they measured, and a pin holds the retirement there. This row measures nothing and commits no prediction: it is the instrument 11.6 and 13.3 will measure through. The suite is 1274 unit tests, 35 end to end tests and 498 Python tests of which 2 skip, all green. Both figures are now what the runners collect rather than what a grep counted: rows 10.1g2 and 10.0b1 landed on this branch while this row was being written, and their entries sit below this one. Next: 11.0b (the cued overlay, its tones and its export rows).

**A GUARD THAT WATCHED ONE LINK OF A FOUR-LINK CHAIN, 7 September 2026 (row 10.1g4, ladder D4) — the ratchet held the detector to the published recall while the stepper, the runner and the scorer went unwatched, and widening it found a trap armed for the regression run.** A published recall is made by four things in series: the detector decides where the blinks are, the stepper decides which frames the detector ever sees, the runner drives the clips, and the Python scorer turns detections and annotations into a percentage. `DETECTOR_SOURCES` watched the first and none of the others, so the stepper could be rewritten and the scorer's arithmetic changed with the result file staying green over a run that described neither. It now watches all four, plus `package-lock.json`, because a browser or Playwright bump can move a stepped measurement without one line of this repository changing. Widening it retroactively made the ratchet red against exactly four commits, each now declared at a stated strength: `b048251` proved neutral (nine added lines, all inside a docstring, no executable line touched), `9d8932b` and `394f0ff` argued from reach with every path enumerated, and `4e64877`, the stepper rewrite, recorded plainly as ABLE to move the numbers and handed to 10.8a, because any other verdict would be the hand-wave the file exists to forbid. Each verdict was written by one reader and then attacked by a second told to refute it; all four stood, and the reader on `9d8932b` checked by loading both versions of the scorer and diffing their output rather than by reading the diff. That check found the trap: `tools/resultGuard.mjs` reads the headline with expressions ending at `found)` and `invented)`, 10.10c1 adds a clause inside those parentheses, and because this file keeps superseded runs below and the parser takes the FIRST match, a regenerated block would have it read precision from the 20 August run, 81.4% and 78 invented in place of 84.0% and 65, while recall stayed current because that block carries an identical recall line. The README would then publish a current recall beside a superseded precision with every byte comparison green, both sides reading the same wrong line. Reproduced here, recorded in the caveat, and given its own row 10.0b2, because it fires exactly when the owner pastes in 10.8a's regenerated block. The same pass fixed a second reach defect: the declaration search ran over the whole result file, which is mostly a table, so any seven-character hexadecimal run in the data counted as naming a commit; it is now scoped to the caveat block, pinned by tests that refuse a sha or a subject appearing only above it. The suite is 1245 unit tests, 35 end to end tests and 498 Python tests of which 2 skip, all green. Next: 10.0b2, the parser that must read the current block rather than the first match.

**A CONSTANT THAT DECIDES A PUBLISHED NUMBER, HELD BY WHAT IT DOES, 7 September 2026 (row 10.1g3, ladder D4's tolerance half) — four frames of forgiveness sets every recall this project has published, and nothing in the suite held it.** `DEFAULT_TOLERANCE_FRAMES` is four: a detection within four frames of a human-marked blink counts as that blink. Raising it to six would have lifted the published recall with nothing turning red. The obvious guard, an equality against 4, passes on a constant nobody reads and says nothing about what four means, so the pin is a pair of behaviours instead: four frames apart matches, five frames apart does not, both calling `match_blinks` with no tolerance argument so the shipped default is what is under test rather than a literal copied into the test. A third test holds the constant to the duration the module's own comment claims, catching a change that leaves the prose behind. The Check is met exactly: the pin reddens at 3, at 5 and at 6, and is green at 4. Row 10.1g is split; this is its tolerance half, and the widened watch list, the caveat-block sha check and the model-provenance pins remain. One methodological note recorded in LEARNING: 3, 4 and 6 give source files of identical length, and three edits inside one second leave the modification time unchanged at the granularity Python compares, so the interpreter reused cached bytecode and reported a restored constant as still mutated. A mutation run over a one-character change in this container has to clear the bytecode cache between steps. The suite is 1240 unit tests, 35 end to end tests and 498 Python tests of which 2 skip, all green. Next: 10.1g's watch-list widening, whose caveat entries are being argued commit by commit.

**THE SAME LIE, ONE LANGUAGE OVER, 7 September 2026 (row 10.0b1, ladder B15) — the published unit figure counted calls in the source where the runner counts cases, and the fix had to stand outside the suite to see it.** `actualUnitTestCount` counted `it(` and `test(` calls and returned 1213; `vitest list` collects 1238, because a table written as a loop around one call is one call and many cases. This is the twin of the defect row 10.1g2 fixed on the Python side hours earlier, and it was visible only because that row's README sentence had to admit it. Same shape of fix, different home: the number lives in `test/collected-tests.txt`, `tools/resultGuard.mjs` reads it, and `npm run counts:check` asks vitest for its own listing and holds the file to it. That check cannot be a test, because counting cases means loading every test file and a test doing so would be counting the run it is inside, so it is a build step running beside the suite in continuous integration, which is what the row's own Check asked for when it said a hand-edited count must redden the build. `unitTestCallCount` keeps the old grep, named for what it counts, and a test asserts the collected number exceeds it. Row 10.0b is split: this is 10.0b1, and the stamp pins, the CHANGELOG guard, ADR-0005 and the STATUS block remain. Tests first, watched failing; the mutation that puts the grep's figure back into the file reddens the guard test and the build step together. The suite is 1245 unit tests, 35 end to end tests and 495 Python tests of which 2 skip, all green. Next: 10.0b2 (resultGuard's stamp pins), or 10.1g (the ratchet watching the whole chain).

**A COUNT OF THE FUNCTIONS IS NOT A COUNT OF THE TESTS, 7 September 2026 (row 10.1g2) — the published Python figure is what the runner collects, and two continuous integration jobs that never meet now hold each other to it.** `actualPythonTestCount` grepped `def test_` across analysis/tests and returned 451; `uv run pytest` collected 493, because a parametrised function expands into one test per case. Every document published the grep's figure and called the result all green, which is a claim about a run that never had that many tests in it, and the gap had grown from 38 to 42 as parametrised tables were added. A grep cannot know the expansion without evaluating Python, and the checks job that reads this number has no Python at all, so the number now lives in `analysis/collected-tests.txt`: `tools/resultGuard.mjs` reads that file, and `analysis/tests/test_collected_count.py` holds the file to what pytest collects, asking it in a subprocess rather than trusting a run that may have been filtered to one file. The old grep is kept as `pythonTestFunctionCount`, named for what it counts, and a test asserts the collected count exceeds it, so replacing the file with a computation reddens the suite instead of going quietly green. Tests first: both sides watched failing, and the mutation that puts the grep's figure back into the file reddens the Node guard and the Python contract together. The same defect is still live on the JavaScript side, where the unit figure counts `it(` calls rather than the cases vitest runs; that is ladder B15 under row 10.0b, and README now says so instead of implying otherwise. The suite is 1215 unit tests, 35 end to end tests and 495 Python tests of which 2 skip, all green. Next: 10.2b (a Content-Security-Policy closing the telemetry residual at the network layer).

**AN ANSWER WITHOUT ITS QUESTION, 7 September 2026 (row 10.13a, ladder A8) — every blink number is measured from a crossing of a line, three different lines can be in force, and no export said which.** A blink is counted when the aperture crosses the blink line; the duration, the amplitude and the peak velocity are all measured from that crossing. Half the passively learned baseline, a stored guided line and a fixed fallback constant can each be that line, and on one face they differ by millimetres. Two exports of one session could disagree about every blink number while agreeing about every column meant to explain them, and `plot.py` drew a confident dashed threshold by multiplying the baseline by a half, which is the passive line and is the wrong line for anybody who had calibrated. `blinkLineMm`, `blinkLineSource`, `shutLineMm` and `shutLineSource` are appended by the trailing-append discipline, so every older header stays an exact prefix and every file already exported still loads. `src/core/lineProvenance.ts` decides nothing: the wiring already picks the line and this reports the pick, because a second opinion about the line would be a second line. The interesting decision was what `none` means. It is not a gap: a frame with no line is the measured fact that nothing was compared, and the loader keeps that apart from a file written before the column existed, which reads as missing because it did not measure no line, it did not say. The same distinction settled two more. A stored line read back on a clip is REFUSED, because a guided line is a measurement of a person at a camera and a clip has neither, so it would measure a stranger against this visitor's eyelids; the calibrate button is gated on a live camera, closing the other direction, where three seconds of a recorded stranger's eye became the visitor's stored line. But a stored line whose camera has moved is only FLAGGED, because the millimetre goes through each frame's own iris ruler and a person who shifted in their seat still has a line in the same units. The margin for that flag is the one number here that could have been invented and is not: `poseBiasSpan()` says the validity gate already accepts head angles across which the published millimetre swings 16.369 percent, so a condition change smaller than an error the instrument already tolerates is not worth a flag, and a test holds the typed constant to the simulation it came from. One defect was found rather than built: `docs/calibration-refusal.txt` has always said numbers depending on the blink line are withheld rather than guessed and listed blink durations among them, and the record withheld the rate and wrote the durations — four places decided withholding separately and they disagreed, and they share one decision now, pinned by a guard watched reddening on the exact one-line mutation because a refusal needs a real face at a real camera and the end to end suite cannot reach one. The stored shape carries its camera, frame size, iris ruler and timestamp now, and `parseBlinkCalibration` stays strict, so an entry stored before today is refused whole and a person who calibrated earlier calibrates once more: deliberate, and a cost, and written down. Eleven `guided_*` metadata rows, and a dated section in `docs/validation-plan-round2.md` on reading a guided session. Five Python test helpers broke at once, every one of them building a CSV row by counting cell positions with the count kept in a comment; they build by column name now and write for a named header generation rather than trimming the current one. The Eyeblink8 byte-identity prediction was committed first in its own commit and is 10.8a's to score. The suite is 1213 unit tests, 35 end to end tests and 451 Python tests of which 2 skip, all green. Next: 11.0a (the cued blink-and-closure schedule, pure).

**THE FLOOR AND THE GRID IT WAS WRITTEN ON ARE THE SAME SIZE, 7 September 2026 (row 10.10c4e, ladder B12) — the stillness figure every millimetre claim is read against is two quanta of the fixture's own rounding, and neither half can be separated from the other.** The recorded fixture does not store what the landmarker produced. Each coordinate is rounded to four decimals before it is written, which cuts the file to a fifth of its size, and until now the published noise floor of 0.020 mm did not say how much of itself was the eye and how much was that grid. `src/core/storageQuantum.ts` converts one stored step into aperture millimetres through the fixture's own measurements rather than through assumptions about it: at 1280x720 the step is 0.072 px vertically and 0.128 horizontally, the lid chords the aperture averages run 13.2 degrees off vertical at the median so the finer vertical grid is the one they ride, the crossover where the coarser horizontal grid would take over is 29.4 degrees, and the fixture's median iris width is 43.3 px. One quantum is 0.0097 mm, so the per-eye medians are 2.03 and 1.99 quanta. The audit's phrase "two rounding quanta" is confirmed as measured rather than recorded as approximate. What that ratio does NOT mean is now stated everywhere it is stated, because the number invites the wrong conclusion: rounding is deterministic, so a perfectly still eye rounds to the same value every frame and would show no wobble at all, and a grid adds nothing to something that is not already moving across it. What it does establish is that the floor and the grid are the same size, so from this file no part of the 0.020 mm can be assigned to the instrument rather than to the storage in either direction. The floor remains an upper bound on the instrument's own wobble, which is the direction that matters, and a reader comparing a 0.1 mm claim against it now knows it cannot be sharpened without a fixture recorded at full precision, which is the ladder's own separate row and needs a recording session rather than arithmetic. Two of the prediction's five parts were wrong and both were one mistake: the iris band of 25 to 40 px was reasoned from the phrase "a webcam close-up" instead of measured from the file sitting in the repository, and the quantum inherited the error, landing at 0.0097 rather than inside the predicted 0.010 to 0.017. Recorded as one mistake counted once. `FIXTURE_COORDINATE_QUANTUM` is exported from the recorder now and the rounding uses it, so a published figure derived from a copy of that number cannot drift away from the rounding it describes. The suite is 1187 unit tests, 35 end to end tests and 447 Python tests of which 2 skip, all green. Next: 10.13a (the blink line travelling with the number it produced, which lands before 10.8a). Not 10.11, which would have used the very floor this row just qualified to derive its re-arm fraction: it is on the corpus-rule list and lands after 10.8a, the regression run at HEAD that only the owner can do.

**THE RULER AND THE THING IT MEASURES DO NOT TURN TOGETHER, 7 September 2026 (row 10.10c4c, ladder B12) — every published millimetre swings 16 points across head angles the gate accepts, and nothing said so.** `apertureMm` is a VERTICAL lid opening divided by a HORIZONTAL iris width. The division cancels distance from the camera, which is what lets the number be compared across sessions at all. It cancels nothing about head angle: a nod foreshortens the numerator, a turn foreshortens the denominator, and they move the ratio in opposite directions. `src/core/poseBias.ts` feeds a synthetic eye of known geometry through the real function with the real index sets, and at the gate's own limits the published millimetre reads 6.03 percent LOW at 20 degrees of pitch and 10.34 percent HIGH at 25 of yaw — a span of 16.4 points on a quantity whose baseline ceiling refuses a whole session for a 25 percent disagreement. Roll is free to nine decimals, which is why it can afford to be the loosest limit of the three. The gate is a refusal, not a correction, and that is now stated where the gate is defined, where the aperture is computed, and in SPEC.md's conventions. Two of six predictions moved, and both only surfaced because the prediction was committed first. The combined case cancels LESS than cos over cos: a nod pushes the lid chord's ends apart in depth and a turn swings some of that depth into width, so the chord picks up a sideways component and the true factor is sqrt(cos²pitch + sin²pitch·sin²yaw) over cos(yaw), 1.0490 rather than 1.0368. And the audit's "up to +12%" for yaw is NOT reproduced: this orthographic projection gives 10.34 at the gate's limit and 12 needs 26.8 degrees, which the gate refuses, so it is recorded as not reproduced rather than adopted, with the perspective term named as the likeliest home for the difference. Nothing is corrected: dividing the chord by cos(pitch) needs a frame-level pose estimate trustworthy enough to divide by, and this row's job was to say how large the uncorrected error is. Row 10.10c4c was split again, the fixture's storage precision becoming 10.10c4e. This row also repaired a red main it had caused, and the repair is the more useful half. A rebase merge rewrites a commit's sha, so the caveat written in the previous pull request named a commit that stopped existing when it landed: the detector ratchet was green on the branch and red on main, and would have been for every change touching a detector source. The ratchet now accepts a commit's SUBJECT as well as its sha, matched against whole lines, because the first version searched the whole document and a subject that is the opening of another line would have counted as declaring that other commit — a guard accepting the wrong declaration. The result file had already written down that a rebase merge does not let a pull request know its own future name; nobody had connected that sentence to the comparison a few hundred lines away that depended on the name. All of this was invisible here until the shallow clone was unshallowed: the ratchet skips itself on a shallow history, so it had never once run in this environment, and yesterday's note claiming the local checklist merely needed running twice is corrected in LEARNING.md. The suite is 1155 unit tests, 35 end to end tests and 446 Python tests of which 2 skip, all green. Next: 10.10c4e (what the fixture's 1e-4 storage quantum is worth in millimetres beside the noise floor it sits under).

**THE PREDICTION WAS WRONG, WHICH IS WHAT PREDICTIONS ARE FOR, 7 September 2026 (row 10.10c4b, ladder B12) — a short blink on a slow machine publishes under half its true closing speed, and the time constant drowsiness lengthens is lengthened the same way by the frame rate.** Peak closing velocity is the fastest drop between two adjacent samples divided by their interval: one forward finite difference, which measures the average speed over an interval containing the fastest instant rather than the instant. A blink's closing phase is 50 to 120 milliseconds and the interval at 25 frames per second is 40, so the two are the same order and the millimetres-per-second unit does not save it. `src/core/velocityBias.ts` samples a raised-cosine descent with a known analytic peak through `analyzeClosing`, the page's own arithmetic rather than a second copy of it, over 40 sampling phases per cell. At 25 fps a 50 ms closure reads 36.8 percent low at the median phase and 60.2 at the worst; amplitude over velocity, which survives sampling in its numerator and not its denominator, reads up to 1.58 times longer than the truth. Drowsiness lengthens that ratio and so does a slow computer, by a comparable amount, so a tired person on a fast laptop and an alert person on a slow one move the published number the same way. The prediction was committed first in its own commit, as this repository requires of anything it measures, and one of its five parts was WRONG: the predicted floor was 10 percent and the measurement came in at 5.4, because I had estimated the loss as if one interval landed wherever it fell while the real code takes the largest drop among ALL adjacent pairs and so already picks the best-placed interval, which recovers most where there are most intervals to choose between. Recorded as wrong rather than quietly adjusted, and the sentence explaining why would never have been written had the prediction not been committed first. The audit's reproduced 12 to 28 percent sits inside the measured spread, so the two do not disagree. Two corrections to the ladder: the sampling interval is already attached to every row as the `fps` column, so what was missing was the bound rather than the interval; and 12 to 28 is a subset of the real spread, not its edges. Nothing is corrected, only bounded: a correction needs a descent model fitted to real lids and is roadmap C6. The suite is 1142 unit tests, 35 end to end tests and 446 Python tests of which 2 skip, all green. Next: 10.10c4c (the pose bound and the fixture's storage precision).

**A BOUND WITH NO SCOPE IS A BOUND ABOUT NOTHING IN PARTICULAR, 7 September 2026 (row 10.10c4a, ladder B12) — the PERCLOS sampling bound covered half the population it was claimed for, and the number it bounds is not the number it is named after.** The page prints "the sampling term is at most ±0.002 of the share even at 15 frames per second", from a real simulation over a committed table, which is more than most such claims have. That simulation drew closures of half a second and longer. An ordinary blink is a fifth of a second, and this instrument's shut line counts every full blink as closed time, so most of the resting share is made of closures the simulation never drew — and short closures are the ones a slow frame rate misjudges most, there being fewer frames inside them. The number was not wrong; the sentence was, claiming for the whole share a bound measured on part of it. The table is untouched, because it measured what it measured, and the sentence now names the scope and the blinks outside it, with a source pin so that lowering the simulated floor without rewriting the sentence goes red. The second bound had almost been stated. MODEL_CARD already said this PERCLOS uses an adjusted closed line and is not comparable to another system's; it did not say the larger reason, which is that the literature's PERCLOS excludes blink frames on purpose and this one cannot, because the line it uses is the line every blink crosses. At rest the figure is therefore mostly blink time and inherits blink-rate variance, so someone who blinks twice as often reads twice as high with eyelids no droopier. That is now stated in `perclos.ts` where the number is defined, in MODEL_CARD beside the caveat it is larger than, and in `drozy-result.txt` where a null about it is read. Row 10.10c4 was split into four: the velocity bound is 10.10c4b, the pose and fixture-precision bounds 10.10c4c, and 10.10c4d NEEDS THE OWNER — the iris constant's citation must come from a paper somebody has read, and inventing a plausible reference for the constant every millimetre on this page divides by would be worse than the silence it replaced. One correction to the ladder: the scope is half a second, not 0.3, because `MIN_CLOSURE_S` is 0.5 and the honest statement is the range that was simulated. The suite is 1129 unit tests, 35 end to end tests and 446 Python tests of which 2 skip, all green. Next: 10.10c4b (the peak-velocity bias, generated from the committed sweep).

**A BAR IS WORTH WHAT IT IS HARD TO CLEAR, 7 September 2026 (row 10.10c2, ladder B9 and B11) — DROZY's within-subject bar is cleared half the time by nothing at all, and three published verdicts rested on it alone.** This project fixes its pass marks before it looks at the data, which is why its nulls are believable. One of those marks turns out not to be one. A feature may be said to track sleepiness only if a corrected p clears a threshold AND at least 3 of the 5 multi-session subjects agree on the direction of the effect. Under the null each subject is a coin flip on direction, so three or more of five is (10 + 5 + 1) / 32, exactly 0.500, and three features were called suggestive and unconfirmed on that half alone. `binomial_at_least` computes it, `analyse_drozy.py` prints it beside every row and beside the verdict rule from its next run, and the result file states it in prose, because regenerating that table would need a re-extract from DROZY.zip that recreates the derived video DATASETS.md requires destroyed. A test recomputes the rate from the counts the committed table already publishes. The rule is fixed forward rather than changed after the fact: any bar added after this states its chance rate and may not grant a verdict alone unless that rate is below 0.05. The second correction is a sentence rather than a number. The alertness comparison found the learned model 0.083 AUC above the heuristic inside a control that could only have detected about 0.14 at this n, and the result file called that "not distinguishable from chance" and "about as good as a fitted one". Both read as a finding of no difference, and no such finding was made; both now read as no evidence either way with the detectable edge named. Absence of evidence stated as evidence of absence is the same mistake as a percentage published without its interval one increment earlier, and both come from reporting what the arithmetic returned rather than what it could have detected. Row 10.10c2 was split again: the subject-level bootstrap intervals are 10.10c2b and NEED THE OWNER, since both resample by subject, the committed result files carry pooled numbers only, and `analyse_rldd.py` and `compare_alertness.py` each take a measured corpus directory that is not in this repository. 10.10c3 needs a corpus run for the same reason. The suite is 1126 unit tests, 35 end to end tests and 444 Python tests of which 2 skip, all green. Next: 10.10c4 (the six known and unstated bounds, whose velocity bound comes from the committed sweep in docs/blink-sample-rate.txt).

**A PERCENTAGE WITH NO INTERVAL IS A NUMBER PRETENDING TO BE AN ANSWER, 7 September 2026 (row 10.10c1, ladder B8 and B10) — every counted proportion this project publishes now carries the interval its denominator supports.** The detector found 341 of the 408 blinks a human marked; the validation round's first criterion was cleared by 0 sessions of 3. Both were published as a bare percentage, which invites a reader to take the number as the answer rather than as what the evidence supports. `wilson_interval` in `analysis/blinklab/stats.py` and `wilsonInterval` in `tools/wilson.mjs` compute it on both sides of the border, held to one another by `test/fixtures/wilson-cases.json`, which both suites read and recompute; they are not identical arithmetic and are not asked to be, one taking its normal quantile from the standard library and the other from a published series, so the table pins them to a millionth, four orders finer than the one decimal any sentence shows. Wilson rather than the textbook normal approximation, and the case that decided it is the second headline above: on 0 of 3 the normal approximation returns exactly 0 to 0, three observations reported as certainty, and it does the same at the top, where 3 of 3 comes back as 100 to 100. Wilson asks which true rates would have made this count unsurprising, so 0 of 3 is 0 to 56 percent and 3 of 3 is 44 to 100. The README's recall and precision now read 79.7 to 86.9 and 80.1 to 87.2, `evaluate_eyeblink8.py` prints both beside its counts, and criterion 1 states the bound its three sound sessions support. One number deliberately carries none: F1 is a harmonic mean of two proportions rather than a count over a count, so an interval there would be arithmetic borrowed from a distribution it does not have, and the temptation to use a new tool on everything in the block is now a test. `evaluate_eyeblink8.py`'s report had no test file at all, the same gap `validation_report.py` had two rows ago. The committed result files were not regenerated, there being no corpus here, so they gain the printed interval at the next measurement while the README carries it now, computed from the counts they already publish. Row 10.10c was split into four before starting, with no item moved. The suite is 1126 unit tests, 35 end to end tests and 431 Python tests of which 2 skip, all green. Next: 10.10c2 (subject-level bootstrap intervals on the scores).

**THE OTHER SIDE OF THE SAME ROUNDING, 7 September 2026 (row 10.1f6, ladder D6's fixture half) — the risk threshold at 60 is pinned now, as the refusal floor at 25 has been since 10.15.** The export writes `sampled_fps` to one decimal, so a measured 59.96 reaches the file as 60.0. Read from the file that is "the evidence rate was 60.0 frames per second, above the 60 risk band". Read from the raw double it is "quick or shallow blinks can be missed below 60". One session, two sentences, and `pilot.py` halts a whole cohort when the page and the mirror disagree like that — which is exactly what happened at the refusal floor in the dry run. The floor was fixed and pinned in 10.15; the risk threshold was the same defect one step up and was pinned nowhere. A `risk-edge` fixture now sits at 59.96, generated by 10.1f5's builders rather than typed, and both implementations reproduce its committed verdict. The mutation was run: handing the page the raw double instead of the rate as the file has it reddens both boundary fixtures, where before it reddened one. The Python byte pin also stopped naming its fixtures. It had four names typed into four tests, so a fixture added on the generating side could be pinned there and never re-derived here; it now finds every `*-session.csv` on disk, with a floor under the count so an empty list cannot pass for agreement. One thing found while counting: the published Python figure is test FUNCTIONS, not tests. The counter reads `def test_` and the runner collects 455, a gap of 42 that parametrised tables widened, while every document says "N Python tests, all green" as though it described the run. That is roadmap row 10.1g2. The suite is 1119 unit tests, 35 end to end tests and 413 Python tests of which 2 skip, all green. Next: 10.2b (a Content-Security-Policy closing the telemetry residual at the network layer).

**TWO DESCRIPTIONS OF ONE THING ARE ONE DESCRIPTION TOO MANY, 7 September 2026 (row 10.1f5, ladder D6's fixture half) — the verdict fixtures come out of the exporter now, and a renamed key reddens the pin that used to sleep through it.** The pin holds two implementations to one file: the page derives its verdict from page state, the Python mirror derives it from the session CSV, and both must reproduce the committed JSON byte for byte. The hole was in the middle. The CSV was typed by hand and the page-state literals beside it were typed by hand again, so the file the two implementations agreed about was one the exporter could not have written, and nothing in the loop imported the metadata writers at all (audit G-export/l-9). The comment above those literals conceded it in as many words. `test/support/verdictFixtures.ts` now describes each session once: the CSV comes from the twelve row builders `exportSession` calls, in the order read out of `main.ts`, and the page-state inputs come from the same object through the real `asExported` and a replay of the real `rulerFitStep` over the same records the file carries, so nothing is left to keep in step. Three drifts the hand-typed files had already accumulated: `kss_before: 3` where `kss.ts` can only emit `3 (Alert)`, bare newlines where `serializeRecords` writes CRLF, and a header two columns behind. Regeneration is `npm run fixtures:write`. The mutation was run in both directions rather than assumed: renaming `pose_valid_fraction` reddens all four fixture byte comparisons where before it left the whole pin green, and regenerating under that rename then reddens five Python tests, the mirror refusing to reproduce a verdict from bytes that lost a key. One rename, two red suites, through one file. A smaller thing worth keeping: the first attempt at that mutation was a one-line search and replace that silently changed nothing, because that call wraps across lines — the exact trap an increment three days earlier documented in this repository. Checking that a mutation mutated something is one line of work and is not optional. The suite is 1118 unit tests, 35 end to end tests and 414 Python tests of which 2 skip, all green. Next: 10.1f6 (the 59.96 boundary, the other side of the rounding pin).

**A READER THAT DEFAULTS WHERE IT SHOULD REFUSE IS A CALM WRONG ANSWER, 7 September 2026 (row 10.1f4, ladder D6's reading half) — every read site's absence policy is written down and exercised, and one of them was the writer's opposite.** The increment before this split the 57 keys by when they are written; this is the other end. A reader meeting an absent key has three honest answers — a default, where absence means the thing did not happen; an unknown, where this one question cannot be answered and the others can; a refusal, where the file is not what it claims to be — and until now which one it took was a property of whichever line happened to be typed. `analysis/tests/test_absence_policy.py` states all nineteen with the judgement in words and exercises each: the reader is called with a block it can read, then with the same block minus one key. The floor matters as much as the assertion, because a reader that raised on everything would satisfy a stated refusal and one that returned its default on everything would satisfy a stated default. `round2._number` and `verdict._finite` now each name the other and say why they diverge: a verdict is a claim about one session, so an input it cannot read means the claim cannot be made, while a round-2 table row survives a blank cell and refusing there would throw away a participant over one absent rate. One policy was stated backwards. `Session.records_dropped` said the row was on every export so a missing key meant an older build; 10.1f3 had exercised the writer that same day and found it written only when rows were dropped, so a missing key means nothing was lost. The reader and the writer described one key and disagreed, both written in the same week. Two read sites merge an absent row with an unreadable one, and both are now named as choices rather than left silent. The mutations were run rather than assumed: a refusal turned into a default and a default turned into a different default each redden exactly the site they belong to. One thing in the ladder could not be done as written and is now its own row, 10.1f4b: `loader.py` cannot refuse a file missing an always-written key without abandoning files this repository still reads, since its own committed session fixture carries two metadata keys of 57 and two test modules load it. Which generations the analysis track still accepts is the owner's decision about their data. The suite is 1115 unit tests, 35 end to end tests and 414 Python tests of which 2 skip, all green. Next: 10.1f5 (the verdict fixtures generated from the real builders).

**ABSENT AND UNKNOWN ARE DIFFERENT CLAIMS, 6 September 2026 (row 10.1f3, ladder D6's presence half) — the when-written column is exercised against the real writers, and it was wrong about seven of 57 keys four hours after it was written.** The contract from 10.1f1 says which keys the export writes; it said nothing a reader could act on about WHEN, and the distinction is the one that decides how to treat an absence. A key every export carries and that turns up missing is a damaged file. A key written only when the thing it describes happened and that turns up missing is an ordinary session, and a reader that refuses it refuses good measurements. `test/core/metadataPresence.test.ts` builds three sessions — a thin camera session where nothing optional happened, a stepped clip, and one where everything optional happened at least once — calls the twelve real row builders `exportSession` calls, and holds SPEC.md's when-written column to what comes out. The builder list is read out of `main.ts` rather than copied, because a copy drifts and a builder added to the export would otherwise leave its keys unclassified with every assertion still passing. `analysis/tests/test_metadata_contract.py` splits its list into the 21 keys every export carries and the 36 written on a condition, each condition in words, and holds the first to that same column, so the Python readers' notion of a promise is transitively held to the exporter. Seven drifts, six of them one mistake: `line()` writes `unknown` rather than dropping a row, so `camera`, `clip_duration_s`, `kss_before`, `kss_after`, `measurement_frame` and `median_iris_width_px` had been described as sometimes-absent when they are always written with a value that may be unknown. The seventh was the opposite and landed on a key 10.1f2 had just given a reader: `feature_records_dropped` is written only when rows were dropped, and calling it unconditional would invite a loader to refuse a healthy session. Two value formats fell out of the same pass, `median_iris_width_px` at one decimal and `clip_duration_s` at three. Tests first, watched failing: 14, and the mutations were run rather than assumed — one changed cell reddens two tests here and one in Python, and a writer that stops writing a row unconditionally reddens three. The suite is 1115 unit tests, 35 end to end tests and 409 Python tests of which 2 skip, all green. Next: 10.1f4 (the absence policies at the read sites).

**A NUMBER AVERAGED ACROSS SESSIONS IS A NUMBER ABOUT ONE INSTRUMENT, 6 September 2026 (row 10.1f2, ladder D6's honesty half) — the three keys nothing read now have readers, and the round report says which builds recorded it.** Every export has carried the commit that built it since remediation E2 and no tool read it, so the published round table could have averaged across a detector change and printed one number with no note, while the file that would have said so sat unread in every session it described. `protocol` was in the same position, and `feature_records_dropped` — the difference between a short session and a truncated one — was read as nothing at all. All three reach `Session` now. `cohort_commits` and `cohort_commit_line` give the round report three answers rather than two: one build named, several named with the warning that the average describes more than one instrument, or the plain statement that these files predate the stamp so the question cannot be answered. That line sits above the tables, because a caveat printed under a number is a caveat read after it. An unreadable dropped-row count is refused when the file is opened rather than when a caller happens to ask, and a missing key and a key saying zero give different answers, because zero is a measurement. Two things about the work itself: `validation_report.py`, which builds the published table, had no test file of its own, so every function in it was reached only through the modules it calls and a change to the report's own assembly could not go red anywhere; and the first version of the new test staged its fixture by prepending an `app_commit` line to a file that already had one, and the loader refused it by name for declaring a key twice, which is row 10.16's shared metadata reader catching a carelessly written test in an unrelated increment. The byte-frozen round I report was regenerated and carries the new line. One correction to the ladder's list of four cohort tools: `analyse_drozy.py` and `analyse_rldd.py` analyse DROZY and UTA-RLDD feature records rather than browser exports, so they carry no `app_commit` at all and a commit line there would promise a field that cannot exist. Tests first, watched failing: 16, ten in `test_loader.py` and six in a `test_validation_report.py` that did not exist. The suite is 1102 unit tests, 35 end to end tests and 405 Python tests of which 2 skip, all green. Next: 10.1f3 (the absence policies and the drifted verdict fixtures).

**A CONTRACT TEST IS WORTH EXACTLY WHAT ITS READER READS, 6 September 2026 (row 10.1f1, ladder D6's contract half, B16) — the 57 metadata keys are written down and asserted from both languages, and it took three readers to get one that reads.** The eighteen data columns have been held from both sides since Phase 7 and have never drifted; the metadata block above them had no such test, and a renamed key left both suites green and turned a Python gate into a pass-through, the reader finding nothing, taking its default and reporting the session as fine (D6). `analysis/tests/test_metadata_contract.py` now reads the keys out of the six writers and holds them to a hand-written EXPECTED_WRITTEN and a READ_BY_PYTHON list, with a grep demanding every `metadata.get("...")` literal in the tree appear in the second. SPEC.md gains "### The session metadata block", 57 rows of key, when written, value format and reader (B16), plus the two FeatureRecord fields and the two CameraStates it was behind on, and its record block is now held to the CSV columns. Writing the test was the easy half. The reader took three attempts and each failure is the same failure in a different costume: `line("key"` on one line misses the thirteen calls that wrap, `sampled_fps` among them, which is the key the session verdict is derived from on both sides of this border; `# key: value` appears in the comments that describe the format, so the second reader reported a metadata key called `key`; and the formatter pads the table's columns, so a pattern that assumed the cells were flush found one key of 57 and the comparison it fed still ran. Every reader here now carries a case it must find, fed to it directly, and a floor on how many keys it finds in the real repository, because a reader that returns nothing is indistinguishable from a repository that contains nothing. The last finding is the reason there are two readers rather than one: renaming `sampled_fps` reddened three Python tests and left the entire TypeScript suite green, so `tools/metadataKeys.mjs` reads the same writers from this side and holds them to the same table. The mutation was then run again and both sides go red. Three corrections to the ladder's own text, recorded there: the writers are six not five, the keys are 57 not about 34, and `csv.ts` writes none of its own today. The suite is 1102 unit tests, 35 end to end tests and 390 Python tests of which 2 skip, all green, and the mutation runner catches all 29. Next: 10.1f2 (the keys nothing reads, and the fixtures that drifted).

**A GUARD IN ONE WORKFLOW CANNOT STOP A PUBLISH IN ANOTHER, 6 September 2026 (row 10.1e, ladder D5) — Pages now waits for CI, publishes only a run that passed, and stamps the commit that run tested.** Every check this repository has runs in the CI workflow, and the deploy workflow published on every push to main (G-Build d-1): two independent reactions to the same event, of which the publish is the faster, so the live site went up about four minutes before anything had been checked and a merge that turned out to be red was already the page a visitor would load. The deploy triggers on `workflow_run` of CI now, completed, on main, with `workflow_dispatch` kept for a person pressing the button, and the build job is guarded on `conclusion == 'success'`. That guard is the half that matters: `types: [completed]` fires on failure and cancellation too, so making the change without it would have been strictly worse than leaving the defect alone, a red CI publishing anyway but later and behind the appearance of a gate. Two things had to be carried by hand once the deploy reacts to a workflow rather than to a commit: checkout takes `github.event.workflow_run.head_sha`, because the default is the branch tip and two merges a minute apart are enough for those to differ, and the build step sets `GITHUB_SHA` to the same value, because `vite.config.ts` reads it to stamp the build's commit into the page and a page reporting a commit it was not built from is worse than one reporting none. A workflow file is never executed by the suite, so nothing in the suite could have noticed any of that: `tools/deployGuard.mjs` reads both files and holds the trigger set, the success condition, the ref, the stamp, and the CI workflow's own name against the string the deploy watches for, since a rename on one side alone would stop every deployment silently. Tests first, watched failing: 8 of them, including two on the reader itself, which refuses a workflow whose `on:` block it cannot parse rather than reporting no triggers and passing. The live half of the check is observable only after this merges: the next deployment should carry this commit's own short sha. The suite is 1095 unit tests, 35 end to end tests and 378 Python tests of which 2 skip, all green, and the mutation runner catches all 29. Next: 10.1f (the metadata contract across the language border).

**FOUR SMALL PIECES OF ARITHMETIC, 6 September 2026 (row 10.16, ladder A27) — the two delivery rates share a span, PERCLOS needs samples as well as seconds, a permutation p can no longer read as zero, and one metadata reader replaces three.** None of these is large and all four were producing sentences that could not be true. The delivered rate was measured between the first and last DELIVERED frame in the window and the sampled rate between the first and last READ, two spans, and then printed as a part-of-whole (F-105): a camera that sped up mid-window could publish "10 frames per second, of which this instrument read 30". Both are counted inside ONE span now, the reads' span, a read remembers when its frame arrived so a frame delivered before the span is not credited to it, and the frames read in a span are a subset of the frames delivered in it, so the read fraction is at most one by construction rather than by a clamp. The equal case gets its own words: "read all 30" instead of the same number printed twice. PERCLOS's minimum observation was a span rule only, satisfied by two samples fifteen seconds apart, which published an eyes-closed share of exactly 50 percent from two frames (F-093): `PERCLOS_MIN_SAMPLES` is 100, chosen far below the 375 a 25 fps session produces in fifteen seconds so it refuses only the degenerate case, and both floors are written into every export so a reader of a file knows what a filled `perclos` column cleared. `light_response.py` returned `at_least / PERMUTATIONS`, so a split no shuffle reproduced printed p = 0.0000, a claim that the observed value is impossible by chance (F-091); it adds one to both parts as `stats.py` and `rldd.py` already did, and the floor is 1/1001. And `frameTrace.ts` wrote its own `# frames_measured` row beside the truncation warning while the caller's coverage rows already carried that key, so a truncated trace declared one key twice, which is what `loader.py` and `validation.py` refuse as an edited or damaged file (G-export/l-7); the row is gone, and the `_read_metadata` that was copied into three modules — refusing a repeated key in two of them and accepting one in `blink_log.py` — is now one `read_metadata` each caller hands its own exception to. Tests first, watched failing: the sped-up-camera case staged as a state, a sweep of five delivery rates against six processing rates asserting `sampled <= delivered`, 99 and 100 PERCLOS samples across a long enough span, a p that must be 1/1001 rather than 0, one `frames_measured` line where the caller owns the key, and the blink log refusing a repeated key. Two classification tests read the stored verdict off the state instead of through a ratio the new floor rightly refuses. The suite is 1087 unit tests, 35 end to end tests and 378 Python tests of which 2 skip, all green, and the mutation runner catches all 29. Next: 10.1e (Pages waits for CI).

**THE GUARDS THAT WERE NOT GUARDING, 6 September 2026 (row 10.1c, ladder D2, D16) — the refusal constants get literal probes, the mutation runner runs again and runs in CI, and four guards that read a third of their door now read all of it.** The September audit bent `MIN_BLINK_FPS` to 23, 24, 28 and 30, and moved six other floors an order of magnitude, with the whole suite green (F-018): every test derived its probes FROM the constant, so the probes moved with it and pinned nothing. Literal boundary probes now sit in each owning test file: 24.9/25/25.1 for the blink floor, the 60/65 hysteresis pair in both directions including the held middle, 14 999/15 000 ms for PERCLOS's observed span, last+1999/2000/2001 for its staleness rule, 99/100 samples and 29 999/30 000 ms for the baseline, 29/30 for the guided phases. `tools/mutationCheck.mjs` had been unrunnable since 20 August (F-019): it bent `BASELINE_RISE_MIN_SAMPLES`, removed by 171b5f4, so it exited "STALE LIST" before checking anything while REMEDIATION.md said it was runnable. The RISE entries are gone, six entries for the frame-rate and guided constants are added, and every entry now names the ONE test file that must go red, which took each mutation from about ninety seconds to about one and turned a local gate nobody ran into a CI step. It is a sharper check as well, and the sharpening found something the same hour: narrowing each entry to its owning test turned 10 of the 27 from caught into SURVIVED. Nine were the pose gate, whose owning test derived every probe from the limits, so pitch could open to 89 degrees or shut to 1 and only some distant fixture noticed; the tenth was the blink refractory, whose cases stood 20 ms and 180 ms from a 150 ms boundary. Both owning tests now carry literals, and the two entries that had been pointed at the wrong file are pointed at the right one. A mutation caught by an unrelated snapshot three folders away is not evidence that a constant is pinned; it is evidence that the constant leaks into a fixture. Four small guard repairs (D16): `claimGuard` matched fixed strings, so the retired absolute claim was banned in the one spelling it was written in while PROJECT.md restated it one word away, with "the device" for "your device" and no exception at all, which is stronger than the sentence that was measured false — it matches a family of wordings now, and PROJECT.md says what actually happens; `exportGuard` read backtick calls only, so the development build's fixture recording, 478 face-mesh points of a real person, downloaded under a plain quoted name and sat outside `.gitignore`, which now refuses `session-*.json` with a negation for the one committed consented fixture; `guardsArmed.test.ts` demands a sibling test per guard, generator and ratchet in `tools/`, with the count stated so adding one is a conscious act; and both workflows install with `npm ci --ignore-scripts`, which is per invocation, so `npm run build` still runs `prepare-assets` and the MediaPipe wasm files still populate. The pull request template gains a line for the mutation check. The suite is 1079 unit tests, 35 end to end tests and 377 Python tests of which 2 skip, all green, and the mutation runner catches all 27 in about two minutes. Next: 10.16 (four small arithmetic fixes).

**A GENERATOR THAT FREEZES A SENTENCE IS SLOWER HAND-MAINTAINED PROSE, 6 September 2026 (row 10.0a3, ladder B5) — every generated claim is now a parse or a quote, and the miss mechanism reads as the measured ceiling the record settled four days ago.** The two generators exist so published prose cannot drift from the record it describes, and both carried sentences that were neither parsed from a document nor pinned to one (F-021). The clearest was the miss mechanism: `tools/cannotSeeBlock.mjs` emitted "The mechanism is unexplained" into the participant report, the README and the model card as bare template text, and went on emitting it for four days after `docs/iris-occlusion.txt` explained it on 2 September, with its test green throughout because the test asserted the template. The claim now reads the ceiling count out of that record (about 49 of the 67) and says what the measurement found: on those closures the eyelid aperture and the iris shape are both flat, so this face model does not register them and no threshold move or signal fusion can recover a signal that is not there. Every cannot-see claim carries the exact sentences it rests on as data, `claimsWithoutPins` names any that carry none, and `buildCannotSeeModule` refuses to emit them; the pins stay in the generator and never reach the browser, because the page ships under a bundle budget. The results block has no claim objects, so it is held the other way: its limitations bullet was the one emitted line with no interpolation in it, and it now carries the alertness AUC and p value and the DROZY session count parsed from their own files, while `unpinnedLiterals` reads the generator's own source and names any bullet that stops parsing. README's miss paragraph had been duplicated, half of it twice, and it is one paragraph that states the mechanism with the numbers behind it. Tests first, watched failing: 6 of them, including one that feeds the reader a bad bullet so a green result cannot mean a broken pattern. Two mutations reddened their tests: a claim stripped of its pins, and the bullet returned to a typed literal. One smaller thing cost twenty minutes and is written down: the generated file is compared byte for byte AND formatted, so the generator now makes the same line-breaking choice the formatter would. With this, row 10.0a is complete in all three of its sub-rows. The suite is 1058 unit tests, 35 end to end tests and 377 Python tests of which 2 skip, all green. Next: 10.1c (refusal constants pinned at their boundaries and the mutation runner revived).

**A PARAGRAPH THAT COUNTS THINGS WILL BE WRONG BY THE THIRD ONE, 6 September 2026 (row 10.0a2, ladder B2, B18) — the Privacy section is generated from the storage the code declares, the export says what it carries before anyone emails it, the browser string is reduced by default, and the font licences ship in full.** README said two things were kept on your device and finished with "those two keys are the only storage this app touches" (F-023). There are four: the blink line arrived in August and the pseudonym in September, each with tests, an entry in the page's own list and a line in the erase control, and each time the paragraph was left alone because nothing could fail when it and the code disagreed. `tools/privacyBlock.mjs` now generates that section from `STORED_ITEMS` and the export disclosure, its test rebuilds the block and a second test looks for every stored key inside the Privacy SECTION rather than anywhere in a long file, and the count in the first sentence is counted. The export carried a camera label, a browser string, a core count, screen and window sizes, two sleepiness answers and a pseudonym, and said none of it where a person about to email the file would read it (F-064): `core/exportContents.ts` builds the sentence by asking the metadata row builders which keys they write, refuses when a key it names has left them, and renders in the Session box beside the export buttons. The browser string itself was the whole `navigator.userAgent`, which names the browser build, the engine build and often the operating system patch level, none of which the analysis reads: `core/userAgent.ts` reduces it to a browser, a major version and a platform family, that is what `user_agent` now carries, the full string is a checkbox that is off by default, and a `user_agent_form` row says which form the file got. The shipped licence notice attributed MediaPipe 1.0.0 while the page ships 1.0.1, and summarised the font licences where it promised them verbatim (G-Build d-2, d-6): the version is held to `lockfileVision(root).version`, the OFL 1.1 ships in full with a copyright line per family, and the test reads the families out of `src/styles.css`, so a fourth typeface with no attribution is a red build. Tests first, watched failing: 24 of them across five files, plus one Playwright sequence proving the disclosure and the checkbox stand in the Session box rather than existing only as tested strings, which is the 9.4b lesson. The suite is 1053 unit tests, 35 end to end tests and 377 Python tests of which 2 skip, all green. Next: 10.0a3 (the generators stop freezing prose), then 10.1c.

**FIVE SENTENCES THAT WERE TRUE WHEN THEY WERE WRITTEN, 6 September 2026 (row 10.0a1, ladder B1, B3, B6's dating half, B7, B13) — the card carries the alertness result, the notice names the interception, the gaze claim becomes "not measured", the cross-engine table gets its date, and the security policy names a door that opens.** None of the five was a number and none of them was written carelessly; each was correct once and the thing underneath it moved. MODEL_CARD.md, revised on 5 September, still said the alertness score had never been tested one day after `docs/alertness-score-result.txt` landed and while README.md reported the number (F-014): the card's alertness row now says AUC 0.70 at p 0.001, cohort-level, per person unvalidated, and a new `parseAlertnessResult` reads both figures out of the record so the card is held to the run rather than to whoever edited it last. Its tested-on table gains the two UTA-RLDD reads as the separate reads they are, 54 subjects for the leave-one-subject-out classification and 52 for the alertness comparison, each parsed from its own file by `parseSubjectCount`, and names tablets as untested, because every session behind every number came from a laptop, a desktop or a phone. `DEMO_NOTICE` said the model "does send" usage statistics, which stopped being true on 5 September when the block went in front of `fetch` (F-024): it now says the model tries to send and this page intercepts the request before it leaves the browser, and the seven echoes (README, MODEL_CARD twice, PROJECT, CONTRIBUTING, docs/UI.md and MANUAL item 52) say the same, with README and MODEL_CARD quoting the constant verbatim under a test that collapses whitespace on both sides so a reflow cannot break the pin. "Reliable near the centre, degrades at the corners" was published beside externally validated numbers with no measurement behind it (F-012): both sites say "not measured", name MANUAL item 34 as the only check that exists and row 14.9 as the instrument. The only cross-engine table in the repository carried no date and had been measured on `1de3ae4`, two fixes before the code it appeared to describe (G-Browser-5): it says so now, calls itself a historical record and points at row 13.0, and the results block and the card say "WebKit binary" where the record says WebKit, adding that no engine other than WebKit has measured this corpus. SECURITY.md sent reporters to GitHub's private vulnerability reporting, which is switched off (F-022): it names the contact link the published page already carries and records the date the channel was last verified shut. Tests first, watched failing: 13 of them, the two parsers and their refusals, the card's numbers and both subject counts, the notice quoted verbatim in both documents, the retired gaze claim held out of both, the table's date and commit, and the reporting channel. Row 10.0a was split into 10.0a1, 10.0a2 and 10.0a3 under this repository's two-hour rule before any of it started; no item moved. The suite is 1027 unit tests, 34 end to end tests and 377 Python tests of which 2 skip, all green. Next: 10.0a2 (what is kept and what is exported), then 10.0a3.

**THE PHONE-FIRST DEMO SURFACE, 6 September 2026 (row 14.0b, ladder A6, E1, E4, E5, B19) — the idle page stops claiming a measurement, the caveat is back beside the score, the way in is on screen on a phone, and the light overlay has a touch exit.** Seven surface defects from the audit, none of them a number. The idle page said "Alertness score: measuring..." and "Blinks: 0" before anything ran (F-077): a table in `core/idleStrings.ts` now says "not measuring" where a number would sit and never counts what was never counted, seeded at load and re-applied by `render()` whenever nothing runs and nothing is kept. The short caveat had stopped being rendered (F-026): it is a paragraph under the score again, pinned by an e2e that reads `#box-alertness`. On a phone Start camera sat below the fold (F-027): Source comes first under 1000 px, and a `phone` Playwright project at 375 px asserts the button is in the idle viewport. The light overlay threw on browsers without `requestFullscreen` and had no touch exit (F-028): the request is guarded, a tap anywhere ends the stimulus, and the words come from `lightPhaseMessage` in core, naming both exits; lightResponse.spec.ts covers the no-fullscreen path. The camera path went quiet while the model downloaded and the countdown to the first score lived in the last card (F-078): the status line says the model is loading until it is ready, and the countdown sits under the score. The refusal sentence was unreachable behind the guided line's label, and the stored-data box listed four keys under "Nothing is stored" (F-087, F-106): the refusal is the first branch, and each stored item carries its state. The interface guard read only the box headings: `tools/uiGuard.mjs` now reads every button label out of main.ts and every idle string out of core and holds docs/UI.md to both; three undocumented buttons and the idle table were added to the document under it. Tests first: `lightPhaseMessage`, the idle table, `storedItemState`, the three guard readers and the two document checks, and four Playwright sequences (the phone viewport, the caveat and idle strings, the loading sentence and the countdown, the no-fullscreen stimulus ended by a tap). The suite is 1014 unit tests, 34 end to end tests and 377 Python tests of which 2 skip, all green. Next: 10.0a (the corpus manifest), then 10.1c.

**THE CAMERA THAT CANNOT BE TURNED OFF, AND THE ONE THAT STOPPED, 6 September 2026 (row 14.0d, ladder A5, A18, A26) — a superseded start ends its own stream, the delivery rates are captured at the end, and a frozen camera ends the session by name.** Three defects at the camera seam. A start superseded while the permission prompt was up used to attach its stream and return, leaving a live track nothing owned (F-017): `src/io/camera.ts` now splits into `requestCamera`, `attachStream` and `stopStream`, `beginCamera` checks its run token between request and attach and stops a stream that lost the race, and the picker and the clip input are disabled while a start is in flight. The session's delivery rates were captured by whichever consumer asked first (F-053): the single settled capture now happens at every camera session end, before the observer stops. A camera that stopped delivering left the page running on its last photograph, with the delivery line blaming the browser and rows written from a frozen frame (F-054): `DeliveryState` keeps the moment of the last frame outside the window, `deliveryStaleness` gives the silence a length once a page that could receive frames has received none for a whole window (a return from a hidden tab restarts that clock, so a tab switch stays an interruption), the readout says `no frames in the last 5 s` and reserves the browser's silence for a browser with no callback, the evidence rate is null rather than the display's pace when delivery is observed, a new `cameraStopped` state (in `sessionOver`, refused by the verdict) is reached from the track's `ended` event and from the drained window, and `src/core/recordGate.ts` writes a row only when a frame arrived since the last one. Tests first: `deliveryStaleness` and the observed/stale readout, `recordDue`, the state's sentence and `sessionOver`, the verdict, `test/io/camera.test.ts` on a stubbed getUserMedia, and two Playwright sequences: two picks inside one request's latency leave one live track with the picker and clip input disabled meanwhile, and a frozen fake camera ends the session by name with the record kept. Mutations: a superseded stream left alive and a staleness that never trips each reddened their e2e. The suite is 1001 unit tests, 30 end to end tests and 377 Python tests of which 2 skip, all green. Next: 14.0b (phone-first demo surface), done above.

**SESSION END IS A STATE, 6 September 2026 (row 14.0a, ladder A4, A17, A22) — Stop and a finished clip both land in `ended`, and the record survives it.** Stop used to drop the page into idle, which greyed both exports and let the next Start wipe the records; a finished clip stayed `running` under a flag so the report, which renders only after a session ends, was unreachable for every clip; the after question was never asked on Stop and was re-asked on every export after a Skip; tab switches after the end counted as interruptions at 0.000 s. Now `CameraState` has `ended`, `sessionOver()` names the two states that keep their data, `render()` keeps the picture, traces, exports, heatmap and replay on them and turns the calibrations, the marker and the light stimulus off, the after question is asked once on the transition and exported with `kss_after_at_seconds`, and the visibility listener is gated on running with a null stamp written as `unknown`. Tests first: unit tests per transition (state message, `sessionOver`, `reportAvailable("ended")`, the KSS row, the unknown stamp, the verdict's ordinary outcome), a Playwright stop-then-export sequence (export enabled, the question once, a Skip not re-asked, marker and light off, the way back shown) and clip-then-report assertions on the stepped clip. The suite is 981 unit tests, 28 end to end tests and 377 Python tests of which 2 skip, all green. Next: 14.0d (the camera that cannot be turned off), done above.

**THE REFUSAL SURVIVES THE EXPORT BORDER, 6 September 2026 (row 10.15, ladder A3) — the page and the Python mirror agree on the 25 fps refusal at the boundary.** The export writes `sampled_fps` to one decimal and the Python mirror reads that; the page handed its verdict the raw double, so a measured 24.96 read refused on the page and warned in the file, and pilot.py stopped the cohort calling the disagreement an instrument defect (audit G-export/l-1). One line in main.ts now rounds the rate through `asExported(…, 1)`, as the marker seconds already were, before the verdict sees it. Tests first: a source pin that the page rounds (red on the old wiring), an `edge` fixture at 25.0 that both implementations reproduce byte for byte (test/fixtures/verdict/edge-*), the mechanism stated in the same test (the raw 24.96 lands on refused), and literal boundary probes at 24.9/25/59.9/60 with the self-refuting "25.0 below the 25" sentence pinned out of existence. The suite is 975 unit tests, 27 end to end tests and 377 Python tests of which 2 skip, all green. Next: 14.0a (session end as a state), done above.

**THE ERA RE-WORKED FROM THE AUDIT, 6 September 2026 (ROADMAP amendment 17, accepted) — Phases 10 to 14 re-ordered and extended, Phase 15 added, every confirmed finding traceable to a row.** The maintainer asked for the roadmap to be worked out against six briefs (improve what exists; finish what is partial; add, expand or correct measurements, metrics and eye signals; fix every confirmed defect and inconsistency; everything else in the audit's scope that pushes the software forward; and a complete, multi-layered re-validation on public data and volunteers across devices). The draft was read by two adversarial reviewers, one for traceability (every ladder item A1 to E10 and all 154 findings mapped to a row or a dated decline; 25 corrections, among them three ladder items with no row, six double-mapped items, two dangling row ids and a 10.8a-versus-13.8 ordering contradiction) and one for feasibility against the code (20 corrections, among them: the vendored blendshape head is a landmark-fed network over 146 mesh points, not an appearance head; hysteresis must sit on the arming side so durations do not move; UTA-RLDD refits belong inside 12.18's single read; the stepper's 10 ms calibration probe is a whole period at 100 fps; the assessment pilot cannot carry a KSS claim at n=6). All 45 are folded in. The table now holds 114 open rows across Phases 10 to 15 (29, 14, 24, 15, 20, 12), eight done; oversized rows are split into lettered sub-rows; every row that can move the Eyeblink8 count says so and lands after 10.8a's regression run with a committed prediction (the regression run is row 10.8a; the previous entry's 'row 10.8' meant it); the Phase 12 gate names 10.11, 10.12a-c, 10.13a-b, 10.14, 13.2 and 13.8b; Phase 15's six layers each name their prerequisites and their n. Demoted to ideas on the maintainer's priority: 13.9, 14.7, 14.8; 12.11 and 12.13 on the audit's evidence; 13.11 kept because 13.12 needs its floor; the former 11.5 deleted. No source changed, so the suite stands at 972 unit tests, 27 end to end tests and 376 Python tests of which 2 skip, all green. Next: the first unticked Phase 10 row in ladder order, 10.15 (the rounded sampled_fps at the export border), then 14.0a (session end as a state), 14.0d, 14.0b, 10.0a and 10.1c, while 10.8a waits for the maintainer's machine.

**THE STEPPER REFUSES WHAT IT CANNOT PLACE ON THE CLOCK, 6 September 2026 (ladder A1, the September audit's critical finding) — a stepped clip no longer measures the same frame twice under an invented timestamp.** Prediction first, its own commit (docs/stepper-honesty.txt): the rule, the tolerances and a two-branch prediction for the Eyeblink8 regression run. Then the code. A new pure module, src/core/stepCalibration.ts, holds the rules: every calibration gap must be a whole multiple of the smallest within 1.5 ms (a 29.97 fps clip with millisecond timestamps alternates 33 and 34 ms and calibrates; a 25 ms gap among 40 ms frames is refused by name as a variable frame rate), the step is the mean period those multiples imply rather than the smallest gap (which under-read a quantised clip's period and drifted a frame every ninety, the candidate explanation for the +1 on two published clips), and an inexact landing (a seek the browser never placed on the clip's clock, recorded at the schedule time) is a counted fact that refuses the run past 2 in 100. The stepper (src/io/videoStepper.ts) probes up to twelve frames, counts its inexact landings and returns them; main.ts refuses by name and writes frame_interval_s, frames_sought and inexact_landings into every stepped export; the Eyeblink8 caveat names the change in prose, because the ratchet's watch list gains the stepper only under ladder item D4, which can name the merged commit's sha. The fixture now takes an explicit frame-time list, and the four pre-registered cases are pinned: 33/34 ms and 33/33/34 ms clips measure every frame with zero inexact landings; a 25 ms gap among 40 ms frames refuses at calibration; a 20 fps clip with a half-period glitch calibrates at the glitch and is refused by the landing count, never reported as 40 fps. Six mutations each reddened the suite (the quantum, the tolerance, the old smallest-gap rule, the multiple check, the counter, the refusal). The suite is 972 unit tests, 27 end to end tests and 376 Python tests of which 2 skip, all green. The corpus half of A1's Check awaits the maintainer's regression run (D10, row 10.8), which scores the prediction. Next: ladder A3 (the rounded sampled_fps at the export border), or the maintainer's ruling on amendment 17.
...
failure — 49 are the model ceiling (aperture and iris both flat on
human-closed frames, and the guided calibration REFUSES on exactly that
population by its 30% separation floor), 9 are the long-closure duration
cap, 5 are state-machine drops, 4 are no-face. A better-placed line has
nothing in this corpus to catch. And with no stored calibration — every
corpus run as the runner exists — the change is neutral BY CONSTRUCTION:
loadBlinkCalibration returns null and the baseline path is byte-for-byte
