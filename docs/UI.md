...
| Record yourself         | Disclosure (a closed `details` beside the clip picker, roadmap 14.6). Three steps for bringing a constant-rate clip from the device's own camera app, and the honesty paragraph quoting the pipeline's real variable-rate refusal — the prefix is pinned in core against the function that speaks it, and the committed probe (docs/record-yourself-probe.txt) is cited as the fact's source.                                                                       |
| Probe camera modes      | Button, visible with the camera controls (roadmap 13.7). Asks the live track the three canonical resolution-and-rate trades and renders the honest mode menu below the picture: one row per mode the camera GRANTED, each with a "Use ..." button, the negotiated-rate sentence and the relative ruler sentence. Choosing applies that ask and the export carries `chosen_mode`; the probe itself restores the session's settings, a question rather than a change. |
...
| Looking toward  | `Looking toward: QUADRANT (calibrated)` / `(uncalibrated)` / `off screen (uncalibrated)` / `outside the window (calibrated)` / `paused, head moved since calibration` / `no valid measurement`. With a profile the boundary is the calibrated window, one rule with the heatmap and the export column (roadmap 14.9b) |
