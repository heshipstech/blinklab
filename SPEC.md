...
| `calibration_window_N_seconds`   | One row per guided calibration run        | `START-END`, seconds to three decimals: the span the four reducers were fed null through (11.6a)        | Nothing                                                                         |
| `calibration_windows`            | Sessions that ran a guided calibration    | Integer count of guided calibrations run during the session (11.6a)                                     | Nothing                                                                         |
| `guided_blinks_caught`           | Camera sessions with a stored guided line | Integer 0 to 3 of the verification blinks the line caught, or `unknown` before verification (11.6a)     | Nothing                                                                         |
| `guided_camera`                  | Camera sessions with a stored guided line | The camera that measured the line, or `unknown`                                                         | Nothing                                                                         |
| `guided_closed_median_mm`        | Camera sessions with a stored guided line | Median aperture of the closed phase, millimetres                                                        | Nothing                                                                         |
| `guided_closed_samples`          | Camera sessions with a stored guided line | Integer count of trusted readings the closed median stood on                                            | Nothing                                                                         |
| `guided_conditions_match`        | Camera sessions with a stored guided line | `true`, or `false (frame-size)` / `false (iris-width)` when the live camera no longer matches the stamp | Nothing                                                                         |
| `guided_frame_size`              | Camera sessions with a stored guided line | The frame size the line was measured at, `WIDTHxHEIGHT`                                                 | Nothing                                                                         |
| `guided_iris_width_px`           | Camera sessions with a stored guided line | The iris ruler at calibration to one decimal, or `unknown`: the working distance                        | Nothing                                                                         |
| `guided_line_mm`                 | Camera sessions with a stored guided line | The stored personal blink line, millimetres                                                             | Nothing                                                                         |
| `guided_open_median_mm`          | Camera sessions with a stored guided line | Median aperture of the open phase, millimetres                                                          | Nothing                                                                         |
| `guided_open_samples`            | Camera sessions with a stored guided line | Integer count of trusted readings the open median stood on                                              | Nothing                                                                         |
| `guided_recorded_at`             | Camera sessions with a stored guided line | ISO timestamp of the calibration                                                                        | Nothing                                                                         |
| `guided_separation_ratio`        | Camera sessions with a stored guided line | How far the closed median sat below the open one, three decimals, or `unknown`                          | Nothing                                                                         |
...
| `camera_frame_driver`            | Camera sessions                           | `video-frame-callback` (once per presented frame, 13.8b) or `animation-frame` (the kept fallback)       | Nothing                                                                         |
| `camera_resolution`              | Camera sessions                           | `WIDTHxHEIGHT`, or `unknown`                                                                            | Nothing                                                                         |
| `chosen_mode`                    | Only when a probe-menu mode was chosen    | The menu row's negotiated label, e.g. `1280x720 at 60 fps` (13.7)                                       | Nothing                                                                         |
...
| `cue_N_hold_ms`                  | One row per instruction cue               | Integer milliseconds the instruction stands                                                             | Nothing                                                                         |
| `cue_N_kind`                     | One row per instruction cue               | `blink`, `close3`, `close20` or `lookAway`; rests are the gaps                                          | Nothing                                                                         |
| `cue_N_seconds`                  | One row per instruction cue               | Seconds from the protocol start, three decimals                                                         | Nothing                                                                         |
| `cue_protocol_start_ms`          | Cued-protocol sessions                    | Milliseconds on the record clock                                                                        | Nothing                                                                         |
| `cue_response_window_ms`         | Cued-protocol sessions                    | Integer milliseconds a response may trail its cue, scaled with the run                                  | Nothing                                                                         |
| `cue_time_scale`                 | Cued-protocol sessions                    | `1.000` on a real session; smaller only on the shortened test runs (11.0b)                              | Nothing                                                                         |
| `cues`                           | Cued-protocol sessions                    | Integer count of instruction cues                                                                       | Nothing                                                                         |
...
| `device_pixel_ratio`             | Every export                              | A number, or `unknown`                                                                                  | Nothing                                                                         |
| `face_detected_fraction`         | Every export                              | Share of records with a face, to three decimals                                                         | Nothing                                                                         |
| `face_lost_longest_ms`           | Only when a loss forced a re-acquisition  | Integer ms of the longest continuous trusted-face loss (13.13)                                          | Nothing                                                                         |
| `face_reacquire_resets`          | Only when a loss forced a re-acquisition  | Integer count of landmarker re-acquisitions signalled (13.13)                                           | Nothing                                                                         |
...
| `frame_rate_after`               | Camera sessions                           | Negotiated rate after the 60 fps ask, or `unknown`                                                      | Nothing                                                                         |
| `frame_rate_apply`               | Camera sessions                           | `ok` or `failed`: whether the 60 fps constraint was accepted                                            | Nothing                                                                         |
| `frame_rate_asked`               | Camera sessions                           | The rate the ask requested (60), as an `ideal` constraint                                               | Nothing                                                                         |
| `frame_rate_before`              | Camera sessions                           | Negotiated rate before the 60 fps ask, or `unknown`                                                     | Nothing                                                                         |
| `frame_rate_declared_max`        | Camera sessions                           | `getCapabilities` maximum, or `unknown` where the browser keeps it                                      | Nothing                                                                         |
| `frame_rate_resolution_change`   | Camera sessions                           | `none`, or `WxH -> WxH` when the ask traded resolution — surfaced, never silent (13.2)                  | Nothing                                                                         |
| `frames_measured`                | Every export                              | Integer count of frames the instrument looked at                                                        | `blink_log.py`, `miss_autopsy.py`                                               |
| `frames_missed_while_busy`       | Camera sessions                           | Integer count of presented frames the busy thread skipped, or `unknown` (13.4)                          | Nothing                                                                         |
| `frames_presented`               | Camera sessions                           | Integer count of frames the compositor presented over the observed span, or `unknown` (13.4)            | Nothing                                                                         |
...
| `hardware_concurrency`           | Every export                              | Integer core count, or `unknown`                                                                        | Nothing                                                                         |
...
| `orientation`                    | Every export                              | The screen orientation, or `unknown`                                                                    | Nothing                                                                         |
| `orientation_flips`              | Every export                              | Integer count of screen-orientation changes during the session                                          | Nothing                                                                         |
...
| `screen`                         | Every export                              | `WIDTHxHEIGHT`, or `unknown`                                                                            | Nothing                                                                         |
| `source`                         | Every export                              | `camera` or `file`                                                                                      | `blink_log.py`, `miss_autopsy.py`                                               |
| `user_agent`                     | Every export                              | The reduced browser string, or the full one when asked for                                              | Nothing                                                                         |
| `user_agent_form`                | Every export                              | `reduced` or `full`                                                                                     | Nothing                                                                         |
| `viewport`                       | Every export                              | `WIDTHxHEIGHT`, or `unknown`                                                                            | Nothing                                                                         |
| `visibility_changes`             | Every export                              | Integer count of tab switches during the session                                                        | `validation.py`, `round2.py`                                                    |
| `wake_lock_acquired`             | Camera sessions                           | `true` or `false`: whether the screen wake lock was ever taken (13.1)                                   | Nothing                                                                         |
| `wake_lock_error`                | Camera sessions                           | The last refusal's message, or `none` when nothing was refused (13.1)                                   | Nothing                                                                         |
| `wake_lock_reacquisitions`       | Camera sessions                           | Integer count of locks re-taken after a tab-hide drop or a release (13.1)                               | Nothing                                                                         |
| `wake_lock_supported`            | Camera sessions                           | `true` or `false`: whether `navigator.wakeLock` existed on this browser (13.1)                          | Nothing                                                                         |
