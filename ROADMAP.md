...
- [ ] 7.5 Baseline classifier with a clearly stated train and test split. Check: reproducibility with a fixed seed. Pre-registered in `docs/uta-rldd-plan.md` (Track B, UTA-RLDD), written before any classifier is fit; the classifier itself awaits the owner running the UTA-RLDD videos through the feature extractor, since the source video is not in the repository and its extraction is a corpus-scale job on the owner's machine.
- [ ] 7.6 Leave one subject out evaluation, per subject scores reported. Check: no subject in both splits. Same pre-registered plan, `docs/uta-rldd-plan.md`: leave-one-subject-out is the primary evaluation and the label-shuffle control doubles as a subject-leakage detector.
