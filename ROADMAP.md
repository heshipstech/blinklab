...
- [ ] 9.2 Pupil diameter, the pure estimator (amendment 14). A `core/pupil.ts` that reads a luminance field of the eye region plus the iris centre and radius and returns a pupil diameter in millimetres through the same 11.7 mm iris ruler the aperture uses, or `null` when the contrast, the shape consistency, or the physiological plausibility is not there. No `io`, no camera, no wiring — pure and synthetic-tested. Check: synthetic dark-disc fields at known radii recover the diameter; low contrast, an occluded/partial pupil, an implausible size, and a malformed field each return null.
...
- [ ] 9.4 The light-response experiment, PRE-REGISTERED before it runs — plan committed 4 September 2026 in `docs/pupil-light-plan.md`, before the instrument (9.3b) is even wired: a screen-brightness stimulus (six 20 s dark/bright cycles), the prediction that the pupil constricts to the bright screen committed in writing first, then measured on the maintainer's camera and recorded as readily as a null (or "inconclusive" if the estimator refuses too many frames). The plan tests the INSTRUMENT using the light reflex as ground truth: if the estimate cannot detect even this large, reliable signal, webcam pupillometry via this path is not viable, and that negative stands. Check: the plan is committed before any measurement, and the result page carries the prediction beside the outcome. Awaiting 9.3b and the maintainer's camera.
