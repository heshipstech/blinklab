...
- [ ] 9.4 The light-response experiment, PRE-REGISTERED before it runs (`docs/pupil-light-plan.md`): a screen-brightness stimulus (dark then bright), the prediction that the pupil constricts to the bright screen committed in writing first, then measured on the maintainer's camera and recorded as readily as a null. Check: the plan is committed before any measurement, and the result page carries the prediction beside the outcome.
