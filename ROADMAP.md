...
- [ ] 9.3 Pupil diameter wired: sample the iris-region pixels from the canvas each frame, feed `core/pupil.ts`, and surface the millimetre reading and its null state in the readout and the per-second export. Check: a Playwright/manual visual check, and the export carries the new column with a blank (not a zero) when the estimator refuses.
