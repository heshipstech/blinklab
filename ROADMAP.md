...
- [ ] 7.12 The owner's own sessions, run as a dry run of the whole protocol before anything is sent. THREE devices rather than two, at the owner's suggestion: iPhone 14 Pro Max, MacBook Air, and the Sony A7 IV through a Cam Link 4K. Ran 16 August; all six files read with no refusals. The drift threshold is SET at 15 percent from the measured 0.0, 0.0 and 5.0, and the dry run found a hole no fixture could: a baseline 41 percent longer than its own session's median aperture passed both halves of the baseline check, so the plan gained a second correction and a fifth check. STILL OPEN because the iPhone session is being re-run: its ten deliberate blinks are not between the two marks, and nothing in the exports can say whether that is the protocol or the detector. Check: the threshold was recorded before any volunteer file was opened, and the plan says the check is void if it is not.
