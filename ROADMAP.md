...
- [ ] 7.5 Baseline classifier with a clearly stated train and test split. Check: reproducibility with a fixed seed. Pre-registered in `docs/uta-rldd-plan.md` (Track B, UTA-RLDD), written before any classifier is fit. The classifier CODE now exists — `analysis/blinklab/rldd.py` (a deterministic numpy multinomial logistic regression, standardised and median-imputed inside each fold) and the runner `analysis/tools/analyse_rldd.py` that produces the pre-registered report, both built before the data and tested on SYNTHETIC data only (a separable signal they must find and must survive the label-shuffle, pure noise they must not). What remains is the RUN, still `[ ]`: the owner's UTA-RLDD feature extraction (a corpus-scale job on their machine, the source video not in the repository), then the result recorded under predict-then-verify.
- [ ] 7.6 Leave one subject out evaluation, per subject scores reported. Check: no subject in both splits. Same pre-registered plan, `docs/uta-rldd-plan.md`. `analysis/blinklab/rldd.py` implements it — leave-one-subject-out with per-subject scores and balanced accuracy, the 1/3 majority floor, and the 1000x label-shuffle control that doubles as a subject-leakage detector, all pinned by tests that hold out a whole subject. Awaiting the owner's feature CSVs to run on real labels and report the per-subject scores.
