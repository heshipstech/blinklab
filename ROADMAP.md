...
- [ ] 9.3b Pupil diameter wired: the io layer reads the iris-region pixels from the canvas each frame via getImageData, calls `luminanceField` + `pupilDiameterMm`, and the reading and its null state reach a `pupilDiameterMm` column in the per-second export (a blank, not a zero, on refusal) and the readout. Check: an end-to-end test through the 7.0 upload pipeline shows the column, blank when the estimator refuses; the readout shows the millimetre value or its null state. User-facing (a new export column and a UI readout) and only truly validated on a real eye, which dovetails with 9.4.
