...
- [ ] 7.11 The checks and the published table: blinks between the two marks with their boundary counts, the long closure, the baseline's readiness and drift, face-detected share, iris width per device with its measurement frame, and declared against processing frame rate. Check: a fixture per verdict, and no participant can be dropped without a row saying why.
- [ ] 7.12 The owner's own two sessions, on the Sony A7 IV and on a plain laptop camera, run as a dry run of the whole protocol before anything is sent. The baseline drift threshold is set from those two and written into `docs/validation-plan.md` with both measured values beside it. Check: the threshold is recorded before any volunteer file is opened, and the plan says the check is void if it is not.
