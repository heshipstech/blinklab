...
- [ ] 7.5 Baseline classifier with a clearly stated train and test split. Check: reproducibility with a fixed seed. Pre-registered in `docs/uta-rldd-plan.md` (Track B, UTA-RLDD). The classifier `analysis/blinklab/rldd.py` (a deterministic numpy multinomial logistic regression, standardised and median-imputed inside each fold, reproducible from a fixed seed) and the runner `analysis/tools/analyse_rldd.py` are built, and a PILOT run on fold 1 (12 subjects, 32 usable videos) is recorded in `docs/uta-rldd-result.txt`: a null, the committed prediction confirmed. Left `[ ]` because the plan's power argument rests on the full ~60 subjects and fold 1 is DROZY's scale; the full run (folds 2 to 5) is the maintainer's decision, recorded on the result page when made.
- [ ] 7.6 Leave one subject out evaluation, per subject scores reported. Check: no subject in both splits. Same pre-registered plan, `docs/uta-rldd-plan.md`. `analysis/blinklab/rldd.py` implements it and the fold-1 pilot exercised it end to end on real labels — leave-one-subject-out (no subject in both splits), per-subject scores and balanced accuracy reported, against the 1/3 floor and the 1000x label-shuffle leakage control; the three-class 0.248 and binary 0.614 both collapse to their shuffled nulls (p 0.61, p 0.17). Recorded in `docs/uta-rldd-result.txt`. Left `[ ]` alongside 7.5 until the full 60-subject run is decided.
