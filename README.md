...
> Revised 17 September 2026, against the state of `main` on that date. When this file changes, this stamp changes with it; a test enforces that.

Read in full on 17 September 2026, claims `5260ee3a`. That stamp records a READ, not an edit: it goes stale when a claim in this file changes, and not when a generated block or a count figure moves. This one was made by an automated pass, which is weaker evidence than the maintainer's own read and is labelled so rather than left to be assumed. Roadmap 10.0b6.
