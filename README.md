...
> Revised 11 August 2026, against the state of `main` on that date. When this file changes, this stamp changes with it; a test enforces that.
...
Phases 0 through 6 are complete: foundations, pixels, landmarks, measurement, blinks, gaze and attention, and the rolling state with the demo score. Phase 7, the honest evaluation track, is under way: a Python analysis folder, a session loader and plots, a licensing gate, and video upload mode so a recorded clip runs through the same pipeline as the live camera. That is 509 unit tests, 8 end to end tests locally across two browser engines of which 6 run on every pull request in Chromium, and 97 Python tests of which 2 skip, all green.
